...
from billu import anon, app, config, db, lang, logger, queue, tasks, userbot, yt
...
@lang.language()
async def bot_left_chat(_, m: types.Message):
    left_member = m.left_chat_member
    if left_member and left_member.id == app.id:
        if await db.is_logger():
            try:
                ...
                await app.send_message(
                    config.LOGGER_ID,
                    f"Bot left chat: <b>{chat_title}</b> [ID: <code>{chat_id}</code>]"
                )
