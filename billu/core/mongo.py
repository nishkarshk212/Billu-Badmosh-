...
class MongoDB:
    ...
    async def add_song(self, song_id: str) -> None:
        if song_id not in self.song_ids:
            await self.songdb.update_one(
                {"_id": song_id},
                {"$set": {"_id": song_id}},
                upsert=True
            )
    ...
    async def is_song_stored(self, song_id: str) -> bool:
        if song_id in self.song_ids:
            return True
        doc = await self.songdb.find_one({"_id": song_id})
        if doc:
            self.song_ids.add(song_id)
            return True
        return False
    ...
    async def load_songs(self) -> None:
        self.song_ids = set([doc["_id"] async for doc in self.songdb.find()])
